/*
 * QEMU NCR53C710/SYM53C710/LSI53C710 SCSI Host Bus Adapter emulation
 *
 * Based on lsi53c895a.c which is:
 * Copyright (c) 2006 CodeSourcery.
 * Written by Paul Brook
 *
 * Then the code was:
 * Hacked to support LSI53C710 for UAE by Toni Wilen,
 * and ported back to qemu by (C) 2020, Helge Deller <deller@gmx.de>.
 * Being worked on by Soumyajyotii Ssarkar
 *
 * This code is licensed under the LGPL.
 *
 * Note:
 * NCR53C710 emulation is incorrect, in the sense that it supports
 ...
 */
 ...
 #if 1
 #define DPRINTF(fmt, ...) \
 do { printf("710_scsi: " fmt , ## __VA_ARGS__); } while (0)
 #define BADF(fmt, ...) \
 do { printf("710_scsi: error: " fmt , ## __VA_ARGS__); } while (0)
 #elif defined DEBUG_LSI
 do { qemu_log_mask(LOG_GUEST_ERROR, "710_scsi: " fmt , ## __VA_ARGS__); } while (0)
 #define BADF(fmt, ...) \
 do { qemu_log_mask(LOG_GUEST_ERROR, "710_scsi: error: " fmt , ## __VA_ARGS__); } while (0)
 #else
 #define DPRINTF(fmt, ...) do {} while(0)
 #define BADF(fmt, ...) \
 do { qemu_log_mask(LOG_GUEST_ERROR, "710_scsi: error: " fmt , ## __VA_ARGS__); assert(false);} while (0)
 #endif
 ...
 typedef struct LSIState710 LSIState710;
 ...
 OBJECT_DECLARE_SIMPLE_TYPE(SysBusNCRState, NCR53C710)
 
 struct SysBusNCRState {
     /*< private >*/
 ...
 static void ncr710_reg_write(LSIState710 *s, int offset, uint8_t val);
 static void lsi_execute_script(LSIState710 *s);
 static void lsi_reselect(LSIState710 *s, lsi_request *p);
 
 static void ncr710_dma_read(LSIState710 *s, dma_addr_t addr,
 ...
 static void ncr710_dma_write(LSIState710 *s, dma_addr_t addr,
 ...
 static inline uint32_t read_dword(LSIState710 *s, uint32_t addr)
 {
     ...
     ncr710_dma_read(s, addr, &buf, 4);
 DPRINTF("read_dword addr %#04x  => %#04x!\n", addr, cpu_to_le32(buf));
 ...
 static void ncr710_set_irq(LSIState710 *s, int level)
 {
     printf("NCR710 IRQ !!\n");
 ...
 static void lsi710_request_cancelled(SCSIRequest *req)
 {
     SysBusNCRState *q = NCR53C710(req->bus->qbus.parent);
     LSIState710 *s = &q->ncr;
 ...
 static void lsi710_command_complete(SCSIRequest *req, uint32_t status, size_t resid)
 {
     SysBusNCRState *q = NCR53C710(req->bus->qbus.parent);
     LSIState710 *s = &q->ncr;
     int out;
 ...
 static void lsi710_transfer_data(SCSIRequest *req, uint32_t len)
 {
     SysBusNCRState *q = NCR53C710(req->bus->qbus.parent);
     LSIState710 *s = &q->ncr;
     int out;
         ...
         if ((insn & (1 << 29)) == 0) {
             ...
             if (insn & (1 << 24)) {
                 ...
                 DPRINTF("Load reg 0x%x size %d addr 0x%08x = %08x\n", reg, n,
                         addr, *(int *)data);
                 for (i = 0; i < n; i++) {
                     ncr710_reg_write(s, reg + i, data[i]);
                 }
             } else {
 ...
 static uint8_t ncr710_reg_read(LSIState710 *s, int offset)
 {
     uint8_t tmp;
 ...
 #undef CASE_GET_REG32
     qemu_log_mask(LOG_UNIMP, "read unknown register %02X\n", offset);
     return 0;
 }
 static uint8_t lsi_reg_readb(LSIState710 *s, int offset)
 {
     uint8_t v = ncr710_reg_read(s, offset);
 #ifdef DEBUG_LSI_REG
 ...
 static void ncr710_reg_write(LSIState710 *s, int offset, uint8_t val)
 {
     ...
     switch (offset)
     {
         ...
         qemu_log_mask(LOG_UNIMP, "710 config reg %02x = %02x\n", offset, val);
         //activate_debugger();
         break;
     }
 #endif
 ...
 //                qbus_reset_all(&s->bus.qbus);
     ...
     case 0x2c: /* DSP[0:7] */
         s->dsp &= 0xffffff00;
         s->dsp |= val;
         if (111111 && (s->dmode & LSI_DMODE_MAN) == 0) {
             s->waiting = 0;
             lsi_execute_script(s);
         }
         break;
     ...
     case 0x2f: /* DSP[24:31] */
         s->dsp &= 0x00ffffff;
         s->dsp |= val << 24;
         if (0000000 && (s->dmode & LSI_DMODE_MAN) == 0) {
             s->waiting = 0;
             lsi_execute_script(s);
         }
         break;
     ...
     case 0x3b: /* DCNTL */
         ...
         break;
     default:
             qemu_log_mask(LOG_UNIMP, "write unknown register %02X\n", offset);
     break;
     }
 ...
 static bool ncr_mem_accepts(void *opaque, hwaddr addr,
                             unsigned size, bool is_write,
                             MemTxAttrs attrs)
 {
 if (size !=1 ) { printf("SIZE %d asked at %#lx\n", size, addr); }
     return (size == 1) || (size == 4);
 }
 
 static void sysbus_ncr_mem_write(void *opaque, hwaddr addr,
                                  uint64_t val, unsigned int size)
 {
     SysBusNCRState *sysbus = opaque;
     LSIState710 *s = &sysbus->ncr;
 
     printf("SCSI write %#lx = %#lx\n", addr, val);
     addr ^= 3;
     ncr710_reg_write(s, addr, val);
 }
 
 static uint64_t sysbus_ncr_mem_read(void *opaque, hwaddr addr,
                                     unsigned int size)
 {
     SysBusNCRState *sysbus = opaque;
     LSIState710 *s = &sysbus->ncr;
     ...
     addr ^= 3;
     val = ncr710_reg_read(s, addr);
     printf("SCSI read %#lx -> %#lx\n", addr ^3, val);
     return val;
 }
 
 static const MemoryRegionOps sysbus_ncr_mem_ops = {
     .read = sysbus_ncr_mem_read,
     .write = sysbus_ncr_mem_write,
     // .endianness = DEVICE_NATIVE_ENDIAN,
     .endianness = DEVICE_BIG_ENDIAN,
     .valid.accepts = ncr_mem_accepts,
     .impl = {
         .min_access_size = 1,
         .max_access_size = 1,
     },
 ...
 static void ncr710_scsi_reset(DeviceState *dev)
 {
     SysBusNCRState *q = NCR53C710(dev);
     LSIState710 *s = &q->ncr;
 ...
 static const VMStateDescription vmstate_ncr_scsi = {
     .name = "ncrscsi",
     .version_id = 0,
     ...
     .fields      = (VMStateField []) {
         ...
         VMSTATE_UINT8(istat, LSIState710),
         ...
         VMSTATE_UINT32(scratch, LSIState710),
 ...
 static void ncr710_scsi_realize(DeviceState *dev, Error **errp)
 {
     SysBusNCRState *q = NCR53C710(dev);
     LSIState710 *s = &q->ncr;
     ...
     memory_region_init_io(&q->iomem, OBJECT(q), &sysbus_ncr_mem_ops,
                           q, "ncr710-ioports", 0x100);
     sysbus_init_mmio(sbd, &q->iomem);
 
     scsi_bus_new(&s->bus, sizeof(s->bus), dev, &lsi_scsi_info, NULL);
 //     scsi_bus_legacy_handle_cmdline(&s->bus);
 }
 
 static void ncr710_class_init(ObjectClass *klass, void *data)
 {
     DeviceClass *dc = DEVICE_CLASS(klass);
 
     dc->realize = ncr710_scsi_realize;
     dc->reset = ncr710_scsi_reset;
     dc->vmsd = &vmstate_ncr_scsi;
     set_bit(DEVICE_CATEGORY_STORAGE, dc->categories);
 }
 
 static const TypeInfo ncr710_scsi_info = {
     .name          = TYPE_NCR53C710,
     .parent        = TYPE_SYS_BUS_DEVICE,
     .instance_size = sizeof(SysBusNCRState),
     .class_init    = ncr710_class_init,
 };
 
 static void ncr710_register_types(void)
 {
     type_register_static(&ncr710_scsi_info);
 }

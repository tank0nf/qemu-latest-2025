...
#define NCR53C710_CTEST2_DACK     0x04    /* DMA acknowledge */
#define NCR53C710_DMODE_MAN       0x01    /* Manual start mode */
#define NCR53C710_SSTAT1_WOA      0x08    /* Wide setup active */

/* Short hand constants for better clarity*/
#define PHASE_DO NCR53C710_PHASE_DO
#define PHASE_DI NCR53C710_PHASE_DI
#define PHASE_CMD NCR53C710_PHASE_CMD
#define PHASE_ST NCR53C710_PHASE_ST
#define PHASE_MO NCR53C710_PHASE_MO
#define PHASE_MI NCR53C710_PHASE_MI
#define PHASE_MASK NCR53C710_PHASE_MASK
...
static void ncr53c710_command_complete(SCSIRequest *req, size_t resid)
{
    NCR53C710SysBusDeviceState *m = NCR53C710(req->bus->qbus.parent);
    NCR53C710State *s = &m->state;
    int out;
...
static void ncr53c710_do_command(NCR53C710State *s)
{
    ...
    s->current->req = scsi_req_new(dev, s->current->tag, 
                                   s->current_lun, buf, s->dbc, s->current);
...
again:
    ...
    if (insn_processed > 10000 && !s->waiting) {
        /* Some drivers cause infinite loops. We should break out after 
...
static void ncr53c710_realize(DeviceState *dev, Error **errp)
{
    ncr53c710_scsi_realize(SYS_BUS_DEVICE(dev), errp);
}
...
static void ncr53c710_class_init(ObjectClass *klass, void *data)
{
    ...
    dc->realize = ncr53c710_realize;
    dc->vmsd = &vmstate_ncr53c710_state;
    device_class_set_legacy_reset(dc, ncr53c710_reset);
